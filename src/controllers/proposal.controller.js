...
//         // 1. Fetch Proposal with Request, Client (User) and Assigned Agent (User)
...
//                         {
//                             model: User, 
//                             as: 'AssignedAgent', // The agent assigned to the Request
...
//         }
//         if (!proposal.pdfPath) {
//             return res.status(404).json({ message: "No PDF found for this proposal. Please recreate it." });
//         }

//         // 2. Extract Data safely
//         const clientUser = proposal.Request?.Client?.User;
//         if (!clientUser) {
//             return res.status(400).json({ message: "Client data is missing associated user." });
//         }

//         // Fallback for Agent data if not assigned or missing
//         const agentName = proposal.Request?.AssignedAgent?.fullName || "BizTech Agent";
//         const agentEmail = proposal.Request?.AssignedAgent?.email || process.env.SMTP_USER;
...
//         // 4. Update Status in DB (Transaction not strictly needed here but good practice)
...
//             sentTo: clientUser.email 
//         });

//     } catch (error) {
//         console.error("❌ Failed to send proposal controller:", error);
